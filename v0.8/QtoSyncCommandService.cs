    ...
    public static class QtoSyncCommandService
    {
        ...
        public static QtoExcelResult FullRebuildExcel(string workbookPath)
        {
            ...
            try
            {
                ...
                QtoExcelResult result = adapter.FullRebuild(workbookPath, rows, validation.ReviewItems, logs, BuildDefaultSettings(layout));
                if (result.Success)
                {
        ...
        private static IDictionary<string, string> BuildDefaultSettings(QtoBudgetLayoutSettings layout)
        {
            ...
            settings["版本"] = "v0.8.2";
            settings["說明"] = "由 QtoWirePlugin v0.8.2 在 CAD 指令完成後自動更新數量，並保留 Excel 人工編修欄位。";
