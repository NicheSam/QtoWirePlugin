...
namespace QtoWirePlugin
{
    public class QtoBlockCatalogService
    {
        ...
        public string GetCatalogPathForLegendDwg(string legendDwgPath)
        {
            if (string.IsNullOrWhiteSpace(legendDwgPath))
            {
                throw new ArgumentException("Legend DWG path is required.", "legendDwgPath");
            }

            return GetDefaultCatalogPath(legendDwgPath);
        }
        ...
        private static QtoBlockCatalog NormalizeCatalog(QtoBlockCatalog catalog)
        {
            ...
            if (catalog.CreatedAt == DateTime.MinValue)
            {
                catalog.CreatedAt = DateTime.Now;
            }

            if (catalog.LastScannedAt == DateTime.MinValue)
            {
                catalog.LastScannedAt = catalog.CreatedAt;
            }

            if (catalog.LastSavedAt == DateTime.MinValue)
            {
                catalog.LastSavedAt = catalog.CreatedAt;
            }
            ...
            foreach (QtoBlockCatalogItem item in catalog.Items)
            {
                ...
                if (item.LastScannedAt == DateTime.MinValue)
                {
                    item.LastScannedAt = catalog.LastScannedAt;
                }

                if (item.LastUpdatedAt == DateTime.MinValue)
                {
                    item.LastUpdatedAt = catalog.CreatedAt;
                }

                item.RefreshStatusDisplayName();
            }

            foreach (QtoBlockCatalogMessage message in catalog.Messages)
            {
                if (message != null && message.CreatedAt == DateTime.MinValue)
                {
                    message.CreatedAt = catalog.CreatedAt;
                }
            }
