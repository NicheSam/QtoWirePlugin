...
namespace QtoWirePlugin
{
    ...
    internal sealed class QtoWorksheetData
    {
        ...
        public IList<int> HiddenRows { get; private set; }
    ...
    internal static class QtoSimpleXlsxWriter
    {
        ...
        private static void WriteWorksheetPart(PackagePart sheetPart, QtoWorksheetData worksheet)
        {
            ...
            HashSet<int> hiddenRows = worksheet == null || worksheet.HiddenRows == null
                ? new HashSet<int>()
                : new HashSet<int>(worksheet.HiddenRows);
            ...
            using (XmlWriter writer = XmlWriter.Create(sheetPart.GetStream(FileMode.Create, FileAccess.Write), settings))
            {
                ...
                if (worksheet != null && worksheet.FreezeRows > 0)
                {
                    ...
                    writer.WriteAttributeString("ySplit", worksheet.FreezeRows.ToString(CultureInfo.InvariantCulture));
                    writer.WriteAttributeString("topLeftCell", "A" + (worksheet.FreezeRows + 1).ToString(CultureInfo.InvariantCulture));
                ...
                if (worksheet != null && worksheet.HiddenColumns != null && worksheet.HiddenColumns.Count > 0)
                {
                    writer.WriteStartElement("cols", SpreadsheetNs);
                    foreach (int column in worksheet.HiddenColumns.Distinct().OrderBy(value => value))
                    {
                        if (column <= 0)
                        {
                            continue;
                ...
                if (worksheet != null && worksheet.AutoFilter && rowCount > 0 && columnCount > 0)
                {
                    writer.WriteStartElement("autoFilter", SpreadsheetNs);
                    writer.WriteAttributeString("ref", "A1:" + ColumnName(columnCount) + rowCount.ToString(CultureInfo.InvariantCulture));
                    writer.WriteEndElement();
                }
