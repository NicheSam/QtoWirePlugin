...
namespace QtoWirePlugin
{
    public class QtoPluginApplication : IExtensionApplication
    {
        ...
        {
            QtoSyncCommandService.StartScopeTracking();
            QtoRibbon.CreateRibbonWhenReady();
        }

        public void Terminate()
        {
            QtoSyncCommandService.Shutdown();
            QtoRibbon.Shutdown();
        }
    }
    ...
    public static class QtoRibbon
    {
        ...
        private static bool TryCreateRibbon()
        {
            ...
            panels.Add(CreateBlockLibraryPanel());
            panels.Add(CreateDrawingPanel());
        ...
        private static RibbonPanel CreateBlockLibraryPanel()
        {
            RibbonPanelSource source = new RibbonPanelSource();
            source.Title = "圖塊庫";

            source.Items.Add(CreateButton("插入\n圖塊", "QTO_INSERT_CATALOG_BLOCK", RibbonItemSize.Large));
            source.Items.Add(CreateButton("管理\n圖塊庫", "QTO_BLOCK_LIBRARY_MANAGER", RibbonItemSize.Standard));
            source.Items.Add(CreateButton("更新專案\n圖塊", "QTO_UPDATE_PROJECT_BLOCKS", RibbonItemSize.Standard));
        ...
        private static RibbonPanel CreateDrawingPanel()
        {
            RibbonPanelSource source = new RibbonPanelSource();
            source.Title = "快速繪圖";

            source.Items.Add(CreateButton("連續放置\n設備", "QTO_PLACE_CATALOG_CONTINUOUS", RibbonItemSize.Large));
            source.Items.Add(CreateButton("繪製\n線管", "QTO_DRAW_QTO_PATH", RibbonItemSize.Standard));
            source.Items.Add(CreateButton("轉換舊\n物件", "QTO_CONVERT_LEGACY_OBJECTS", RibbonItemSize.Standard));
        ...
        private static string GetCommandDescription(string commandName)
        {
            switch (commandName)
            {
                ...
                case "QTO_BLOCK_LIBRARY_RESCAN":
                    return "選擇標準圖例 DWG，重建或更新 QTO 圖塊資料庫 catalog。";
                case "QTO_INSERT_CATALOG_BLOCK":
                    return "從 QTO 圖塊資料庫選擇標準圖塊，插入後自動寫入 QTO 基本資料。";
                case "QTO_BLOCK_LIBRARY_MANAGER":
                    return "管理標準圖例 DWG 的圖塊名稱、系統、設備類型、計量方式與預設線材。";
                case "QTO_UPDATE_PROJECT_BLOCKS":
                    return "比較專案圖塊與標準庫版本，預覽差異後再由使用者確認更新。";
                case "QTO_PLACE_CATALOG_CONTINUOUS":
                    return "選一次標準設備後連續放置，插入時自動帶入圖層與 QTO 資料。";
                case "QTO_DRAW_QTO_PATH":
                    return "選擇配線、管段或線槽類型後直接繪製，完成時自動寫入 QTO 長度資料。";
                case "QTO_CONVERT_LEGACY_OBJECTS":
