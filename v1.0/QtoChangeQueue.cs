    ...
    public sealed class QtoChangeQueue : IDisposable
    {
        ...
        public void Enqueue(QtoCadChange change)
        {
            ...
            lock (gate)
            {
                ...
                // AutoCAD changes are normally flushed on CommandEnded. Only start the
                // background debounce timer when a real batch subscriber exists.
                if (QtoChangePerformancePolicy.ShouldUseBackgroundDebounce(BatchReady != null))
                {
                    RestartTimerNoThrow();
                }
            }
