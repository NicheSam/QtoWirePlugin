...
namespace QtoWirePlugin
{
    ...
    public static class QtoRibbon
    {
        ...
        public static void CreateRibbon()
        {
            if (!TryCreateRibbon())
            {
                WriteStatusMessage("\nQtoWirePlugin V0.6 ribbon is not ready. It will be created automatically when AutoCAD finishes loading.");
        ...
        private static void RibbonCreateTimerTick(object sender, EventArgs e)
        {
            ...
            {
                StopRibbonCreateTimer();
                WriteStatusMessage("\nQtoWirePlugin V0.6 could not create the ribbon. Run QTO_SHOW_UI after AutoCAD finishes loading.");
            }
        }
        ...
        private static bool TryCreateRibbon()
        {
            ...
            panels.Add(CreateMepCeilingPanel());
        ...
        private static RibbonPanel CreateBudgetPrepPanel()
        {
            ...
            source.Items.Add(CreateButton("預算\nCSV", "QTO_EXPORT_BUDGET_INPUT", RibbonItemSize.Large));
        ...
        private static string GetCommandDescription(string commandName)
        {
            switch (commandName)
            {
                ...
                case "QTO_PROPERTY_PANEL":
                    return "開啟 v0.6 QTO 屬性面板，挑選圖塊或線條後可查看、編輯、儲存預算前置屬性。";
