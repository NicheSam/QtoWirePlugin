...
namespace QtoWirePlugin
{
    ...
    public static class QtoRibbon
    {
        ...
        public static void CreateRibbon()
        {
            if (!TryCreateRibbon())
            {
                WriteStatusMessage("\nQtoWirePlugin V0.7 ribbon is not ready. It will be created automatically when AutoCAD finishes loading.");
        ...
        private static void RibbonCreateTimerTick(object sender, EventArgs e)
        {
            ...
            {
                StopRibbonCreateTimer();
                WriteStatusMessage("\nQtoWirePlugin V0.7 could not create the ribbon. Run QTO_SHOW_UI after AutoCAD finishes loading.");
            }
        }
        ...
        private static bool TryCreateRibbon()
        {
            ...
            panels.Add(CreateSyncPanel());
            panels.Add(CreateScopePanel());
            panels.Add(CreateBudgetPrepPanel());
        ...
        private static RibbonPanel CreateSyncPanel()
        {
            RibbonPanelSource source = new RibbonPanelSource();
            source.Title = "Excel 同步";

            source.Items.Add(CreateButton("同步\n主控", "QTO_PANEL", RibbonItemSize.Large));
            source.Items.Add(CreateButton("更新\nExcel", "QTO_SYNC_FULL_REBUILD", RibbonItemSize.Large));
            source.Items.Add(CreateButton("檢查\n修復", "QTO_VALIDATE", RibbonItemSize.Large));
        ...
        private static RibbonPanel CreateBudgetPrepPanel()
        {
            RibbonPanelSource source = new RibbonPanelSource();
            source.Title = "預算前置";
        ...
        private static RibbonPanel CreateScopePanel()
        {
            RibbonPanelSource source = new RibbonPanelSource();
            source.Title = "\u6a13\u5c64/\u7cfb\u7d71";

            source.Items.Add(CreateButton("\u6a13\u5c64\n\u6846", "QTO_SCOPE_DEFINE_FLOOR", RibbonItemSize.Large));
            source.Items.Add(CreateButton("\u7cfb\u7d71\n\u6846", "QTO_SCOPE_DEFINE_SYSTEM", RibbonItemSize.Large));
            source.Items.Add(CreateButton("\u6e05\u9664\n\u6846\u7dda", "QTO_SCOPE_CLEAR", RibbonItemSize.Large));
        ...
        private static RibbonPanel CreateMepCeilingPanel()
        {
            RibbonPanelSource source = new RibbonPanelSource();
            source.Title = "\u5929\u82b1\u6a19\u8a3b";

            source.Items.Add(CreateButton("\u521d\u59cb\u5316\n\u5716\u5c64", "MEPDIMINIT", RibbonItemSize.Large));
            source.Items.Add(CreateButton("\u81ea\u52d5\n\u6a19\u8a3b", "MEPCEILDIM", RibbonItemSize.Large));
            source.Items.Add(CreateButton("\u6a19\u8a3b\n\u898f\u5247", "MEPDIMRULE", RibbonItemSize.Large));
        ...
        private static void DrawCommandIcon(DrawingContext context, string commandName, int size)
        {
            ...
            else if (commandName.IndexOf("MEP", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                DrawMepCeilingSymbol(context, whitePen, cyanPen, amberPen, scale);
            }
            else if (commandName.IndexOf("SCOPE", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                DrawScopeSymbol(context, whitePen, cyanPen, amberPen, scale);
            }
        ...
        private static void DrawMepCeilingSymbol(DrawingContext context, Pen pagePen, Pen dimPen, Pen tagPen, double scale)
        {
            context.DrawLine(pagePen, Point(5, 7, scale), Point(27, 7, scale));
            context.DrawLine(pagePen, Point(5, 25, scale), Point(27, 25, scale));
            context.DrawLine(dimPen, Point(8, 11, scale), Point(8, 21, scale));
            context.DrawLine(dimPen, Point(6, 11, scale), Point(10, 11, scale));
            context.DrawLine(dimPen, Point(6, 21, scale), Point(10, 21, scale));
            context.DrawRectangle(null, tagPen, Rect(15, 12, 8, 8, scale));
            context.DrawLine(tagPen, Point(14, 23, scale), Point(24, 23, scale));
        }

        private static void DrawScopeSymbol(DrawingContext context, Pen pagePen, Pen accentPen, Pen tagPen, double scale)
        {
            context.DrawRectangle(null, pagePen, Rect(5, 6, 22, 20, scale));
            context.DrawLine(accentPen, Point(9, 10, scale), Point(23, 10, scale));
            context.DrawLine(accentPen, Point(9, 22, scale), Point(23, 22, scale));
            context.DrawEllipse(null, tagPen, Point(16, 16, scale), 4 * scale, 4 * scale);
            context.DrawLine(tagPen, Point(16, 12, scale), Point(16, 20, scale));
            context.DrawLine(tagPen, Point(12, 16, scale), Point(20, 16, scale));
        }
        ...
        private static string GetCommandDescription(string commandName)
        {
            switch (commandName)
            {
                ...
                case "QTO_EXPORT_BUDGET_INPUT":
                    return "匯出 QTO_BUDGET_INPUT CSV，供工作台產生預算草稿與缺項清單。";
                case "QTO_SYNC_FULL_REBUILD":
                    return "依目前 CAD 圖面更新預算 Excel；若同一路徑 Excel 已開啟，會嘗試直接更新該工作簿。";
                case "QTO_VALIDATE":
                    return "掃描目前圖面並開啟 Review 清單，集中檢查問題、定位物件與處理可修復項目。";
                case "QTO_SCOPE_DEFINE_FLOOR":
                    return "\u5c07\u5c01\u9589 Polyline \u8a2d\u70ba\u6a13\u5c64\u6846\uff0c\u53ea\u6703\u8986\u84cb\u6846\u5167 QTO \u7269\u4ef6\u7684\u6a13\u5c64\u3002";
                case "QTO_SCOPE_DEFINE_SYSTEM":
                    return "\u5c07\u5c01\u9589 Polyline \u8a2d\u70ba\u7cfb\u7d71\u6846\uff0c\u53ea\u6703\u8986\u84cb\u6846\u5167 QTO \u7269\u4ef6\u7684\u7cfb\u7d71\u4ee3\u78bc\u3002";
                case "QTO_SCOPE_APPLY_SELECTED":
                    return "\u5c0d\u9078\u53d6\u7684\u6a13\u5c64\u6846\u6216\u7cfb\u7d71\u6846\u5957\u7528\u898f\u5247\uff1b\u7121 QTO \u8cc7\u6599\u7684\u7269\u4ef6\u6703\u8df3\u904e\u3002";
                case "QTO_SCOPE_APPLY_ALL":
                    return "\u6383\u63cf\u6a21\u578b\u7a7a\u9593\u5168\u90e8\u6a13\u5c64\u6846\u8207\u7cfb\u7d71\u6846\uff0c\u91cd\u65b0\u8986\u84cb QTO \u7269\u4ef6\u7684\u6a13\u5c64\u8207\u7cfb\u7d71\u3002";
                case "QTO_SCOPE_DYNAMIC_ON":
                    return "\u958b\u555f\u52d5\u614b\u6a13\u5c64/\u7cfb\u7d71\u8986\u84cb\uff1b\u547d\u4ee4\u7d50\u675f\u6642\u81ea\u52d5\u4f9d\u7bc4\u570d\u6846\u66f4\u65b0 QTO \u7269\u4ef6\u3002";
                case "QTO_SCOPE_DYNAMIC_OFF":
                    return "\u95dc\u9589\u52d5\u614b\u6a13\u5c64/\u7cfb\u7d71\u8986\u84cb\uff1b\u9700\u8981\u624b\u52d5\u57f7\u884c\u5957\u7528\u5168\u90e8\u6216\u5957\u7528\u9078\u53d6\u3002";
                case "QTO_SCOPE_CLEAR":
                    return "\u6e05\u9664\u9078\u53d6\u6a13\u5c64\u6846\u6216\u7cfb\u7d71\u6846\u7684\u7bc4\u570d\u5c6c\u6027\uff0c\u4e0d\u522a\u9664 Polyline \u672c\u9ad4\u3002";
                ...
                case "MEPDIMINIT":
                    return "\u5efa\u7acb MEP \u5929\u82b1\u6a19\u8a3b\u5c08\u7528\u5716\u5c64\u3001\u6587\u5b57\u6a23\u5f0f\u8207\u5c3a\u5bf8\u6a23\u5f0f\u3002";
                case "MEPCEILDIM":
                    return "\u4f9d\u5e8f\u9078\u53d6 X/Y \u57fa\u6e96\u7dda\u8207\u8a2d\u5099\u5716\u584a\uff0c\u81ea\u52d5\u7522\u751f\u5929\u82b1\u5b9a\u4f4d\u5c3a\u5bf8\u8207\u8a2d\u5099\u6a19\u7c64\u3002";
                case "MEPDIMRULE":
                    return "\u5217\u51fa\u76ee\u524d\u5167\u5efa\u7684 MEP \u5929\u82b1\u6a19\u8a3b\u898f\u5247\uff0c\u4fbf\u65bc\u6aa2\u67e5\u5716\u584a\u540d\u7a31\u5c0d\u61c9\u3002";
                default:
                    return commandName;
            }
        }
