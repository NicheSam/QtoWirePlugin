...
namespace QtoWirePlugin
{
    ...
    public class QtoSyncMainPalette : UserControl
    {
        ...
        public QtoSyncMainPalette()
        {
            ...
            Label subtitleLabel = QtoUiTheme.CreateMutedLabel("依 CAD 更新預算 Excel，保留 Excel 人工編修欄位。");
            headerPanel.Controls.Add(subtitleLabel, 0, 1);
            root.Controls.Add(headerPanel, 0, 0);

            TableLayoutPanel statusPanel = CreateGridPanel(4);
            ...
            lastSyncValueLabel = AddValueRow(statusPanel, 4, "最後同步");
            ...
            actionPanel.Controls.Add(CreateButton("1  更新預算 Excel", FullRebuildButtonClick, QtoButtonRole.Primary), 0, 0);
            actionPanel.Controls.Add(CreateWorkflowText("依 CAD 更新數量，保留 Excel 單價、備註、確認狀態。"), 1, 0);
            actionPanel.Controls.Add(CreateButton("2  檢查問題", ValidateButtonClick, QtoButtonRole.Default), 0, 1);
            actionPanel.Controls.Add(CreateWorkflowText("列出缺資料、ID、圖塊資料庫問題。"), 1, 1);
            actionPanel.Controls.Add(CreateButton("3  修復/人工確認", RepairButtonClick, QtoButtonRole.Default), 0, 2);
            actionPanel.Controls.Add(CreateWorkflowText("可修復的先處理，其餘交給 Review 清單。"), 1, 2);
            ...
            syncSwitchPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50));
            syncSwitchPanel.Controls.Add(CreateButton("開始監看", StartSyncButtonClick, QtoButtonRole.Secondary), 0, 0);
            syncSwitchPanel.Controls.Add(CreateButton("停止監看", StopSyncButtonClick, QtoButtonRole.Secondary), 1, 0);
            actionPanel.Controls.Add(syncSwitchPanel, 0, 3);
            ...
            quickPanel.Controls.Add(CreateSmallButton("設定", SettingsButtonClick));
        ...
        private void StartSyncButtonClick(object sender, EventArgs e)
        {
            ...
            overview.CurrentStatus = result.Success ? "同步中" : "同步失敗";
        ...
        {
            QtoRepairResult result = QtoSyncCommandService.RepairCurrentDrawing();
            AddLog("自動修復完成：" + result.RepairedCount.ToString("0") + " 項。");
            QtoDialogService.ShowRepair(ToRepairPlanItems(result), this);
        }

        private void BlockCatalogButtonClick(object sender, EventArgs e)
        {
            AddLog("已提示圖塊資料庫處理方式。");
            MessageBox.Show(this, "目前請先用「檢查問題」找出未設定圖塊，再回到屬性面板補齊系統代碼與設備類型。圖塊資料庫管理會在下一階段獨立成外部視窗。", "圖塊資料庫", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private static IList<QtoRepairPlanItem> ToRepairPlanItems(QtoRepairResult result)
        {
            ...
            List<QtoRepairPlanItem> plans = new List<QtoRepairPlanItem>();
            ...
            if (plans.Count == 0)
            {
                plans.Add(new QtoRepairPlanItem
                {
                    IssueType = "沒有可自動修復項目",
                    Count = 0,
                    RepairAction = "請查看 Review 清單是否仍有人工確認項目。",
                    CanAutoRepair = false
                });
            }
        ...
        {
            AddLog("已顯示主控面板下方的同步紀錄。");
            MessageBox.Show(this, "目前先在主控面板下方顯示簡要紀錄。", "同步紀錄", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void SettingsButtonClick(object sender, EventArgs e)
        {
            QtoDialogService.ShowSettings(this);
        }
