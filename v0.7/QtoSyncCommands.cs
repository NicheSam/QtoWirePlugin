...
namespace QtoWirePlugin
{
    public partial class Commands
    {
        ...
        public void QtoValidate()
        {
            ...
            Application.ShowModelessDialog(new QtoReviewForm(result.ReviewItems));
            Editor editor = Application.DocumentManager.MdiActiveDocument.Editor;
        ...
        public void QtoRepair()
        {
            ...
            Application.ShowModalDialog(new QtoRepairForm(ToRepairPlanItems(result)));
            Editor editor = Application.DocumentManager.MdiActiveDocument.Editor;
            editor.WriteMessage("\nQTO_REPAIR: 已自動修復 " + result.RepairedCount.ToString("0") + " 項。");
        }
        ...
        {
            Application.ShowModalDialog(new QtoSettingsForm());
        }
        ...
        private static System.Collections.Generic.List<QtoRepairPlanItem> ToRepairPlanItems(QtoRepairResult result)
        {
            ...
            if (plans.Count == 0)
            {
                plans.Add(new QtoRepairPlanItem
                {
                    IssueType = "沒有可自動修復項目",
                    Count = 0,
                    RepairAction = "請回到 Review 清單查看是否仍有人工確認項目。",
                    CanAutoRepair = false
                });
            }
