...
namespace QtoWirePlugin
{
    public partial class Commands
    {
        ...
        private static IEnumerable<string> GetSystemCodeOptions(Document document)
        {
            ...
            List<string> values = dictionary == null ? new List<string>() : new List<string>(dictionary.SystemCodes);
            if (values.Count == 0)
            {
                values.AddRange(new string[] { "LAN", "TEL", "CCTV", "ACS", "BMS", "EMS", "PARK", "PA", "ELV_WIRE", "UNKNOWN" });
            }

            return values.Distinct(System.StringComparer.OrdinalIgnoreCase).ToList();
        }
